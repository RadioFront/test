...
def update_pid_controller_rate_att(action):
    ## Rate Controller
...
def get_telemetry_data():
    ...
    try: 
        ...
        LOCAL_POSITION_NED_data=[LOCAL_POSITION_NED_x/10,LOCAL_POSITION_NED_y/10,LOCAL_POSITION_NED_z/10,LOCAL_POSITION_NED_vx/10,LOCAL_POSITION_NED_vy/10,LOCAL_POSITION_NED_vz/10]
...
class PX4_RL_PID(py_environment.PyEnvironment):
    ...
    def _step(self, action):
        ...
        current_reward = 1/(0.1+self.track_distance/self.sample_size)/self.max_distance
...
while mode!="steady_state":
    ...
    try: 
        ...
        if mode == "flight_mode" and distance_geo([0,0,-10],[LOCAL_POSITION_NED_x, LOCAL_POSITION_NED_y, LOCAL_POSITION_NED_z])<1:
            mode = "steady_state"
            stampMode = time.time()
            print("Mode:",mode)
    except:
...
#action = np.array([0.3, 0.0004, 0.1, 0.3, 0.0004, 0.1, 0.3, 0.04], dtype=np.float32)
...
#time_step = environment.step(action)
#print(time_step)
#action = np.array([0.3, 0.0004, 0.1, 0.3, 0.0004, 0.1, 0.3, 0.04], dtype=np.int32)
#time_step = environment.step(action)
#print(time_step)
#time_step = environment.step(action)
#print(time_step)
...
for _ in range(num_iterations):
    ...
    while mode!="steady_state":
        ...
        try: 
            ...
            LOCAL_POSITION_NED_y = master.messages[t_param].y
            LOCAL_POSITION_NED_z = master.messages[t_param].z
            if mode == "flight_mode" and distance_geo([0,0,-10],[LOCAL_POSITION_NED_x, LOCAL_POSITION_NED_y, LOCAL_POSITION_NED_z])<0.3:
                mode = "steady_state"
                stampMode = time.time()    
